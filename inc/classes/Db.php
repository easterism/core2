...
//        echo "<PRE>";print_r($child_class_name);echo "</PRE>";die;
        ...
        if (
            ...
            $mod_name = preg_match(
                '~^Mod[A-z0-9_]+(Controller|Worker|Cli|Api)$~',
                $child_class_name,
                $matches,
            )
        ...
        if ($mod_name) {
            ...
            $context = $reg->isRegistered("context")
                ? $reg->get("context")
                : ["admin"];
    ...
    public function __get($k)
    {
        ...
        if ($k == "cache") {
            ...
            if (!$reg->isRegistered($k)) {
                ...
                $options = $this->_core_config->cache->options
                    ? $this->_core_config->cache->options->toArray()
                    : [];
                $adapter_name = !empty($this->_core_config->cache->adapter)
                    ? $this->_core_config->cache->adapter
                    : "Filesystem";
                if (isset($this->config->cache->adapter)) {
                    ...
                    if (
                        ...
                        !empty($this->config->cache)
                    ) {
                        //если кеш задан в основном конфиге
                ...
                if ($adapter_name == "Redis") {
                    ...
                    if (
                        !empty(
                            $this->_core_config->cache->options->server
                                ->password
                        )
                    ) {
                        $options->setPassword(
                    ...
                    $options
                        ...
                        ->setTtl(
                            $this->_core_config->cache->options->ttl ?? 172800,
                        );
        ...
        elseif (strpos($k, "data") === 0) {
            if ($reg->isRegistered($k)) {
                ...
                if (
                    $classPath &&
                    strpos($classPath, dirname(strstr($location, "/mod/"))) !==
                        0
                ) {
                ...
                if (!file_exists($location . "/Model/$model.php")) {
                    throw new Exception(
                        $this->translate->tr("Модель $model не найдена."),
                    );
                }
        ...
        try {
            ...
            if ($database->adapter === "Pdo_Mysql") {
                if ($this->config->system->timezone) {
                    $db->query(
                        "SET time_zone = '{$this->config->system->timezone}'",
                    );
                }
                ...
                if (
                    $this->_core_config &&
                    $this->_core_config->profile &&
                    $this->_core_config->profile->on
                ) {
    ...
    public function getModuleName(string $resId): array
    {
        ...
        if (!empty($module_id[1])) {
            if (
                !isset($module["submodules"]) ||
                !isset($module["submodules"][$module_id[1]])
            ) {
                return [];
            } //модуль есть а субмодуля нет
     ...
     * @throws \Exception
     */
    public function logActivity($exclude = []): void
    {
        ...
        if ($auth->ID && $auth->ID > 0) {
            ...
            if (
                ...
                if (!$this->config->log->access->file) {
                    throw new Exception(
                        $this->translate->tr("Не задан файл журнала запросов"),
                    );
                }
    ...
    {
        $this->getAllSettings();
        return isset(Registry::get("_settings")[$code])
            ? Registry::get("_settings")[$code]["value"]
            : false;
    }
    ...
    public function getEnumList($global_id, $active = true)
    {
        ...
        foreach ($res as $id => $value) {
            if ($active && $value["is_active_sw"] !== "Y") {
                continue;
            }
            $data[$id] = $value;
        }
     ...
     * @return array
     */
    public function getEnumDropdown(
        $global_id,
        $name_as_id = false,
        $empty_first = false,
        $active = true,
    ...
    final public function isModuleActive(string $module_id): bool
    {
        ...
        if (!$mod || $mod["visible"] !== "Y") {
            return false;
        }
        if (!empty($id[1])) {
            if (
                empty($mod["submodules"][$id[1]]) ||
                $mod["submodules"][$id[1]]["visible"] !== "Y"
            ) {
                return false;
            }
        }
    ...
    final public function isModuleInstalled($module_id): array
    {
        ...
        if (!Registry::isRegistered("_modules")) {
            return [];
        }
        $is = isset(Registry::get("_modules")[$module_id])
            ? Registry::get("_modules")[$module_id]
            : [];
        return $is;
    }
    ...
    final public function getModuleVersion($module_id)
    {
        ...
        $version = isset(Registry::get("_modules")[$module_id])
            ? Registry::get("_modules")[$module_id]["version"]
            : "";
        return $version;
    }
    ...
    final public function getModuleLoc($module_id)
    {
        ...
        if (!$module_id) {
            throw new \Exception(
                $this->translate->tr("Не определен идентификатор модуля."),
            );
        }
        if (!empty($this->_locations[$module_id])) {
            return $this->_locations[$module_id];
        }
        ...
        if (!isset($module["location"])) {
            if ($module_id === "admin") {
                ...
                if (!$module) {
                    throw new \Exception(
                        $this->translate->tr("Модуль не существует") .
                            ": " .
                            $module_id,
                        404,
                    );
    ...
    {
        $this->getAllModules();
        return Registry::isRegistered("_modules") &&
            isset(Registry::get("_modules")[$module_id])
            ? Registry::get("_modules")[$module_id]
            : [];
    }
    ...
    private function getAllSettings(): void
    {
        ...
        if (empty($settings)) {
            ...
            $settings_rows = $table_settings->fetchAll(
                $table_settings->select()->where("visible = 'Y'"),
            );
            $settings = [];
    ...
    protected function emit($event_name, $data = [], $module_override = "")
    {
        ...
        $reg = Registry::getInstance();
        if (!$reg->isRegistered("emitter")) {
            $reg->set("emitter", new Emitter());
        }
