...
class dbf_class {
    ...
    function dbf_class($filename) {
        ...
        if (ord($this->_raw[0])==131) { //See if this has a memo file with it...
            //Read the File
            $tail=mb_substr($tail,-1,1);   //Get the last character...
            if ($tail=='F'){            //See if upper or lower case
                $tail='T';              //Keep the case the same
            } else {
                $tail='t';
            }
            $memoname = mb_substr($filename,0,strlen($filename)-1).$tail;
    ...
    function getRow($recnum) {
        ...
        $rawrow = mb_substr($this->_raw,$recnum*$this->_rowsize+$this->_hdrsize,$this->_rowsize);
        $rowrecs = array();
        $beg=1;
        if (ord($rawrow[0])==42) {
        ...
        for ($i=0; $i<$this->dbf_num_field; $i++) {
            $col=trim(mb_substr($rawrow,$beg,$this->dbf_names[$i]['len']));
            if ($this->dbf_names[$i]['type']!='M') {
                $rowrecs[]=$col;
            } else {
                $memobeg=$col*512;  //Find start of the memo block (0=header so it works)
                $memoend=mb_strpos($this->_memos,$memoeot,$memobeg);   //Find the end of the memo
                $rowrecs[]= mb_substr($this->_memos,$memobeg,$memoend-$memobeg);
            }
    ...
    function getRowAssoc($recnum) {
        ...
        for ($i=0; $i<$this->dbf_num_field; $i++) {
            ...
            if ($this->dbf_names[$i]['type']!='M') {
                ...
                $memobeg=$col*512;  //Find start of the memo block (0=header so it works)
                $memoend= mb_strpos($this->_memos,$memoeot,$memobeg);   //Find the end of the memo
                $rowrecs[$this->dbf_names[$i]['name']]=mb_substr($this->_memos,$memobeg,$memoend-$memobeg);
            }
            $beg+=$this->dbf_names[$i]['len'];
        }
        return $rowrecs;
