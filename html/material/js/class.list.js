...
var listx = {
                    ...
                    $.post('index.php?module=admin&action=switch&loc=core',
                        ...
                        function(data, textStatus) {
                            if (textStatus == 'success' && data.status == "ok") {
                                ...
                                if (data.status) {
                                    if (alertify) alertify.alert(data.status);
                                }
                            }
                        },
                        'json'
	...
	buttonAction : function(id, url, text, nocheck, obj, callback) {
		...
		if (!url) {
            if (alertify) {
                alertify.alert('Временно недоступна.');
            } else {
		...
		if (!val && !nocheck) {
			if (alertify) {
				alertify.alert("Вы должны выбрать как минимум одну запись.");
			} else {
	...
	del: function (id, text, isAjax) {
		...
		if (val) {
			if (val.length) {
                        ...
                        if (e) {
                            ...
                            if (listx.loc[id]) {
                                $.ajax({
                                    ...
                                    alertify.alert("Не удалось выполнить удаление");
                                }).always(function () {
                                    preloader.hide();
                                });
                            }
                ...
                if (alertify) {
                    alertify.alert('Нужно выбрать хотя бы одну запись');
                } else {
                    alert('Нужно выбрать хотя бы одну запись');
                }
			}
		}
	},
