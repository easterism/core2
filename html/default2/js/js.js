...
function changeRoot(obj, to) {
	if (!obj) return;
	var parent           = obj.parentNode;
	var is_overflow_menu = main_menu.is_overflow();
	for (var i = 0; i < parent.childNodes.length; i++) {
		if (parent.childNodes[i].nodeName == 'LI') {
            parent.childNodes[i].className = is_overflow_menu ? 'menu-module pills' : 'menu-module';
			if (parent.childNodes[i] == obj) {
                parent.childNodes[i].className = is_overflow_menu ? 'menu-module-selected pills' : 'menu-module-selected';
...
function toAnchor(id){
	$('html,body').animate({scrollTop: $("#"+id).offset().top - $("#menu-container").height()}, 'fast');
}
...
var loadPDF = function (url) {
	preloader.show();
    $("#main_body").height($("body").height() - ($("#menu-container").height() + 15));
...
function resize() {
    ...
    $("#main_body").height($("#rootContainer").height() - ($("#menu-container").height() + 15));
}
...
$(document).ready(function() {
    ...
    if (main_menu.is_overflow()) {
        main_menu.addClassModules('pills');
    }
