...
class ModBillingController extends Billing {
    ...
    /**
     * Получение информации о платежах и состоянии баланса
     * @return string
     * @throws WebServiceException
     */
    public function restInfo() {
        ...
        $where = ! empty($_GET['type']) && is_string($_GET['type'])
            ? $this->db->quoteInto("WHERE type_operation = ?", $_GET['type'])
            : '';
        ...
        if ( ! empty($history)) {
            foreach ($history as $operation) {
                if ($operation['paid_operation'] &&
                    ! empty($this->moduleConfig->operations) &&
                    ! empty($this->moduleConfig->operations->{$operation['paid_operation']}) &&
                    ! empty($this->moduleConfig->operations->{$operation['paid_operation']}->title)
                ) {
                    $operation['paid_operation'] = $this->moduleConfig->operations->{$operation['paid_operation']}->title;
                }
            }
        }
