    ...
    public function getFormComing() {
        ...
        if ( ! empty($webpay['options'])) {
            ...
            if (empty($webpay['options']['wsb_return_url'])) {
                $webpay['options']['wsb_return_url'] = $gateway_url . "?action=success&system=webpay&protocol={$protocol}&host={$host}&apikey={$apikey}";
            }
            if (empty($webpay['options']['wsb_cancel_return_url'])) {
                $webpay['options']['wsb_cancel_return_url'] = $gateway_url . "?action=cancel&system=webpay&protocol={$protocol}&host={$host}&apikey={$apikey}";
            }
            if (empty($webpay['options']['wsb_notify_url'])) {
                $webpay['options']['wsb_notify_url'] = $gateway_url . "?action=notify&system=webpay&protocol={$protocol}&host={$host}&apikey={$apikey}";
            }
