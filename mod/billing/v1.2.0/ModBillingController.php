...
class ModBillingController extends Billing {
    ...
    public function action_index() {
        ...
        $title .= 'Баланс: ' . $balance_commafy . ' ' . $currency;
    ...
    public function restSuccess() {
        ...
        if ($this->moduleConfig->check_action_hash &&
            (empty($_SERVER['HTTP_CORE_BILLING_HASH']) ||
             $_SERVER['HTTP_CORE_BILLING_HASH'] != $this->moduleConfig->check_action_hash)
        ) {
    ...
    public function restCancel() {
        ...
        if (empty($operation)) {
            throw new WebServiceException("operation number \"{$_POST['operation_num']}\" not found", 805, 400);
        }
        ...
        if ($operation['status_transaction'] == 'pending') {
            ...
            throw new WebServiceException('the operation has already been performed', 806, 400);
        }
        ...
        if ($system_name == 'balance') {
            switch ($type_operation) {
                case 'expense':
                    ...
                    $billing_operations = new Billing_Operations();
                    $result_expense = $billing_operations->createExpense($_POST['operation_name']);
