...
class ToolsTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        return array(
            array('core2/tests/classes/AlertTest.php'),
            array('core2\\tests\\Init.php'),
        );
    }
    ...
    public function test_httpAuth() {
        ...
        //         opaque='5ccc069c403ebaf9f0171e9517f40e41'";
         ...
         $users = array(
             array('username' => 'Aladdin', 'password' => 'open sesame')
         );
    ...
    public function test_declNum() {
        $number= 360;
        $titles= array('триста шестьдесят','трехсот шестидесяти','тремстам шестидесяти','триста шестьдесят','тремястами шестьюдесятью','(о) трехстах шестидесяти');
        ...
        $number= 40;
        $titles= array('сорок','сорока','сорока','сорок','сорока','(о) сорока');
        $fn = $this->tools->declNum($number, $titles);
        //echo("\n $fn");
        if ($fn != '40 сорока') {
        ...
        $number= 5;
        $titles= array('пять','пяти','пяти','пять','пятью','(о) пяти');
        $fn = $this->tools->declNum($number, $titles);
        //echo("\n $fn");
        if ($fn != '5 пяти') {
    ...
    public function providerString()
    {
        return array(
            array('Stroka'),
            array("абвгдеёжзийклмнопрстуфхцчшщъыьэюя"), //Русский алфавит в кодировке windows-1251
            array("ЮАБЦДЕ╦ФГХИЙКЛМНОПЯРСТУЖВЬЫЗШЭЩЧЪ"),//Русский алфавит в кодировке koi8-r, koi8-u, iso-ir-111
            array('рстуфхИцчшщъыьэюя№ёђѓєѕіїјљњћќ§ўџ'),//Русский алфавит в кодировке iso-8859-5
            array("рстуфх╕цчшщъыьэюяЁёЄєЇїЎў°∙·√№¤■"),//Русский алфавит в кодировке x-cp866
            array("ЯрРсСтИТуУжЖвВьЬ№­ыЫзЗшШэЭщЩчЧ§■"),//Русский алфавит в кодировке ibm855
            array('абвгдеЄжзийклмнопрстуфхцчшщъыьэю€'),//Русский алфавит в кодировке x-mac-cyrillic, x-mac-ukrainian
        );
    }
    ...
    */
    public function test_arrayMultisort(){
        $type = SORT_ASC;
        $a = array(
                array('service' => 1, 'number' => 5, 'name' => 'five'),
                array('service' => 2, 'number' => 4, 'name' => 'four'),
                array('service' => 1, 'number' => 1, 'name' => 'one'),
                array('service' => 7, 'number' => 3, 'name' => 'three'),
                array('service' => 1, 'number' => 6, 'name' => 'six'),
                array('service' => 10, 'number' => 2, 'name' => 'two'),
                array('service' => 11, 'number' => 15, 'name' => 'fifteen'),
                array('service' => 13, 'number' => 12, 'name' => 'twelve'),
                array('service' => 14, 'number' => 11, 'name' => 'eleven'),
                array('service' => 27, 'number' => 13, 'name' => 'thirteen'),
                array('service' => 21, 'number' => 16, 'name' => 'sixteen'),
        );
